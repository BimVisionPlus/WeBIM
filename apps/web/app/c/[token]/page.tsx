...
export default async function PublicCanvasPage({ params }: { params: { token: string } }) {
  ...
  return <main className="min-h-screen bg-[rgb(var(--inverse-bg))] p-4"><div className="mx-auto mb-3 flex max-w-[1500px] items-center text-[rgb(var(--inverse-ink))]"><div><div className="text-xs font-semibold uppercase tracking-widest text-blue-400">Atlas AEC · Hồ sơ trình duyệt</div><div className="text-sm text-[rgb(var(--inverse-ink))]">{data.sheet.projectName}</div></div><div className="ml-auto text-xs text-[rgb(var(--muted-2))]">Không cần tài khoản · {share.role === "COMMENT" ? "Được phép bình luận" : "Chỉ xem"}</div></div><div className="mx-auto max-w-[1500px]"><SheetCanvas sheet={data.sheet} initialMarkups={data.markups} createEndpoint={`/api/canvas/public/${params.token}`} presenceEndpoint={`/api/canvas/public/${params.token}/presence`} canComment={share.role === "COMMENT"} guest /></div></main>;
}
