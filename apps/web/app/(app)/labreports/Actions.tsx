...
export function CreateForm({ projects }: { projects: ProjectOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-white" data-testid="open-create-form">+ Lấy mẫu thí nghiệm</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="labreports-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <label className="text-xs"><span className="block text-slate-600">Mã mẫu</span><input required value={f.sampleCode} onChange={(e) => setF({ ...f, sampleCode: e.target.value })} placeholder="LAB-XXX-001" className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 font-mono" name="sampleCode" /></label>
        <label className="text-xs"><span className="block text-slate-600">Loại</span><select value={f.sampleType} onChange={(e) => setF({ ...f, sampleType: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="sampleType"><option value="BE_TONG">Bê tông</option><option value="THEP">Thép</option><option value="XI_MANG">Xi măng</option><option value="CAT_DA">Cát đá</option><option value="DAT_NEN">Đất nền</option><option value="COC_NEN">Thử tải cọc</option><option value="KHAC">Khác</option></select></label>
        <label className="text-xs md:col-span-2"><span className="block text-slate-600">Phương pháp</span><input required value={f.testMethod} onChange={(e) => setF({ ...f, testMethod: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="testMethod" /></label>
        <label className="text-xs"><span className="block text-slate-600">TCVN</span><input required value={f.tcvnRef} onChange={(e) => setF({ ...f, tcvnRef: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="tcvnRef" /></label>
        <label className="text-xs"><span className="block text-slate-600">Mã LAS-XD</span><input required value={f.labCode} onChange={(e) => setF({ ...f, labCode: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="labCode" /></label>
        <label className="text-xs md:col-span-2"><span className="block text-slate-600">Tên phòng LAS</span><input value={f.labOrgName} onChange={(e) => setF({ ...f, labOrgName: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="labOrgName" /></label>
      </div>
...
export function ResultActions({ id, result }: { id: string; result: string }) {
  ...
  return (
    <div className="flex flex-wrap gap-1" data-testid={`result-actions-${id}`}>
      ...
      <button onClick={() => go("CONDITIONAL")} disabled={busy === "CONDITIONAL"} className="rounded bg-amber-600 px-2 py-0.5 text-[10px] font-medium text-white disabled:opacity-50" data-testid="action-CONDITIONAL">{busy === "CONDITIONAL" ? "…" : "ĐKĐK"}</button>
    </div>
  );
}
