    ...
    <span className="inline-flex items-center gap-2" data-testid={`row-actions-${id}`}>
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/40 p-4" onClick={() => setOpen(false)}>
          <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-lg bg-white p-5 shadow-2xl">
            ...
            <div className="space-y-3">
              ...
              <label className="block text-xs"><span className="text-slate-600">Tỉnh trọng tâm</span><input value={f.province} onChange={(e) => setF({ ...f, province: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" /></label>
              <label className="block text-xs"><span className="text-slate-600">Phạm vi</span><textarea value={f.scope} onChange={(e) => setF({ ...f, scope: e.target.value })} rows={2} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" /></label>
              <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={f.active} onChange={(e) => setF({ ...f, active: e.target.checked })} /><span className="text-slate-700">Đang hoạt động</span></label>
            </div>
