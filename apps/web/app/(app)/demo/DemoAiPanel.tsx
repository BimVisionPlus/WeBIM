    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3">
        {buttons.map((b) => (
          <button
            ...
            className="group flex items-center justify-between gap-2 rounded-md border border-slate-300 bg-white px-3 py-2 text-left text-xs font-medium text-slate-700 hover:border-blue-500 hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      {results.length > 0 && (
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          <div className="mb-2 text-xs font-semibold text-slate-500">Kết quả gần nhất (5 lần gọi):</div>
          <div className="space-y-3">
            {results.map((r, i) => (
              <div key={i} className="border-l-2 border-emerald-400 bg-emerald-50/40 px-3 py-2">
                ...
                <pre className="mt-1 max-h-64 overflow-auto whitespace-pre-wrap text-[11px] leading-relaxed text-slate-800">
                  {r.body.slice(0, 4000)}
                </pre>
              </div>
            ))}
