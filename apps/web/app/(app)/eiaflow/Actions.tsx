...
export function CreateForm({ projects }: { projects: ProjectOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-white" data-testid="open-create-form">+ Tạo hồ sơ ĐTM</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="eiaflow-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <label className="text-xs"><span className="block text-slate-600">Loại</span><select value={f.type} onChange={(e) => setF({ ...f, type: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="type"><option value="DTM">ĐTM</option><option value="DKDT">ĐKĐT</option><option value="GPMT">GPMT</option><option value="BAO_CAO_DK">Báo cáo định kỳ</option></select></label>
        <label className="text-xs"><span className="block text-slate-600">Mã</span><input required value={f.code} onChange={(e) => setF({ ...f, code: e.target.value })} placeholder="DTM-2026-XXX" className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 font-mono" name="code" /></label>
        <label className="text-xs md:col-span-3"><span className="block text-slate-600">Cơ quan thẩm định</span><input required value={f.authority} onChange={(e) => setF({ ...f, authority: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="authority" /></label>
      </div>
