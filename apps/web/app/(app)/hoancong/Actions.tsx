...
export function CreateForm({ projects }: { projects: ProjectOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-white" data-testid="open-create-form">+ Khởi tạo hồ sơ hoàn công</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="hoancong-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <label className="text-xs"><span className="block text-slate-600">Mã</span><input required value={f.code} onChange={(e) => setF({ ...f, code: e.target.value })} placeholder="HC-VHGP-S9-2026" className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 font-mono" name="code" /></label>
      ...
      <div className="mt-3 flex justify-end"><button type="submit" disabled={busy} className="rounded bg-blue-600 px-4 py-1.5 text-sm font-medium text-white disabled:opacity-50" data-testid="submit-create">{busy ? "…" : "Khởi tạo"}</button></div>
    </form>
  );
}
    ...
    <div className="mt-3 flex flex-wrap gap-2" data-testid={`actions-${id}`}>
      {actions.map((a) => (
        <button key={a.action} onClick={() => go(a.action)} disabled={busy === a.action}
          className={`rounded px-3 py-1.5 text-xs font-medium ${a.tone === "danger" ? "bg-rose-100 text-rose-800" : a.tone === "primary" ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-800"} disabled:opacity-50`}
