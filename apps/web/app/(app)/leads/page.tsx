  ...
  return (
    <AecModuleShell group="Phát triển thị trường" name="Dự án đang theo & tiềm năng" subtitle="Pipeline cơ hội đấu thầu — tiềm năng → đang theo → trúng / không trúng.">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Card><CardBody className="py-3"><div className="text-xs text-slate-500">Tổng cơ hội</div><div className="mt-1 text-2xl font-bold">{leads.length}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-slate-500">Tiềm năng</div><div className="mt-1 text-2xl font-bold text-slate-700">{potential}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-slate-500">Đang theo dõi</div><div className="mt-1 text-2xl font-bold text-blue-700">{tracking}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-slate-500">Pipeline value</div><div className="mt-1 text-xl font-bold">{formatVnd(BigInt(pipelineValue))}</div><div className="text-[10px] text-emerald-700">{won} đã trúng</div></CardBody></Card>
      </div>
      ...
      <Card className="mt-4">
        ...
        <CardBody className="p-0">
          {leads.length === 0 ? (
            <div className="p-8 text-center text-sm text-slate-500">Chưa có cơ hội nào trong pipeline. Bấm "Thêm cơ hội" để bắt đầu.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500">
                <tr><th className="p-2 text-left">Dự án / Gói thầu</th><th className="p-2 text-left">Khách hàng</th><th className="p-2 text-left">Địa bàn</th><th className="p-2 text-right">Giá trị</th><th className="p-2 text-left">Trạng thái</th><th className="p-2 text-left">Next action</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {leads.map((l) => {
                  const m = statusMeta[l.status] ?? { vn: l.status, variant: "neutral" as const };
                  return (
                    <tr key={l.id} className="hover:bg-slate-50" data-testid={`row-lead-${l.id}`}>
                      <td className="p-2"><div className="font-medium">{l.name}</div><div className="text-[10px] text-slate-500">{l.source ?? "—"} · {l.province ?? "—"}</div></td>
                      <td className="p-2 text-xs">{l.clientName ?? "—"}</td>
                      <td className="p-2 text-xs">{l.territory?.name ?? <span className="text-slate-400">—</span>}</td>
