...
export function PromoteAction({ leadId, orgId, suggestedKey }: { leadId: string; orgId: string; suggestedKey: string }) {
  ...
  return (
    <form onSubmit={submit} className="inline-flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
      <input required value={projectKey} onChange={(e) => setProjectKey(e.target.value.toUpperCase())} placeholder="Mã DA" className="w-24 rounded border border-[rgb(var(--line-2))] px-1.5 py-0.5 text-xs" data-testid={`promote-key-${leadId}`} />
      <button type="submit" disabled={busy} className="rounded bg-emerald-600 px-2 py-0.5 text-xs font-medium text-[rgb(var(--inverse-ink))] disabled:opacity-50" data-testid={`promote-submit-${leadId}`}>{busy ? "…" : "OK"}</button>
      <button type="button" onClick={() => setOpen(false)} className="text-xs text-[rgb(var(--muted))]">×</button>
