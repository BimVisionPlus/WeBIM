...
const statusMeta: Record<string, { label: string; cls: string }> = {
  ...
  NO_DATA: { label: "Thiếu dữ liệu", cls: "bg-slate-100 text-slate-800 border-slate-300" },
};
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-end gap-2">
        ...
        <button onClick={run} disabled={busy || !projectId} className="rounded bg-blue-600 px-3 py-1.5 text-xs font-medium text-white disabled:opacity-50">{busy ? "AI đang đánh giá…" : "Chạy AI compliance check"}</button>
      </div>
      ...
      {result && (
        <div className="space-y-3">
          <div className={`rounded border p-4 ${statusMeta[result.overallStatus]?.cls ?? "bg-slate-50"}`}>
                ...
                {result.standards.map((s: any) => {
                  ...
                  return (
                    <tr key={s.code} className="align-top">
                      <td className="p-2 text-xs"><div className="font-mono font-medium">{s.code}</div><div className="text-[10px] text-slate-500">{s.title}</div></td>
