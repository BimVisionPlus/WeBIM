  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Tổng lô VL</div><div className="mt-1 text-2xl font-bold">{total}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Đã trả NCC</div><div className="mt-1 text-2xl font-bold text-rose-700">{rejected}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Thiếu CO</div><div className="mt-1 text-2xl font-bold text-amber-700">{noCo}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Thiếu CR hợp quy</div><div className="mt-1 text-2xl font-bold text-amber-700">{noCr}</div></CardBody></Card>
      </div>
      ...
      <Card className="mt-4">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="text-xs text-[rgb(var(--muted))]">{Array.from(byCat.entries()).map(([k, v]) => `${catLabel[k]}: ${v}`).join(" · ")}</div>
          </div>
        </CardHeader>
        <CardBody className="p-0">
          {lots.length === 0 ? (
            <div className="p-6 text-center text-sm text-[rgb(var(--muted))]">Chưa có lô vật liệu. Bấm “Nhận lô VL” để ghi nhận lô đầu tiên.</div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[rgb(var(--line))]">
                {lots.map((l) => {
                  ...
                  return (
                    <tr key={l.id} className="hover:bg-[rgb(var(--raised))]" data-testid={`lot-${l.lotCode}`}>
                      <td className="p-2 font-mono text-xs">{l.lotCode}</td>
                      <td className="p-2 text-xs"><div className="font-medium">{l.materialName}</div><div className="text-[10px] text-[rgb(var(--muted))]">{l.manufacturer}{l.origin && ` · ${l.origin}`}</div></td>
                      ...
                      <td className="p-2 text-[10px]">
                        ...
                        {needsCr && <><br/><span className={l.crCertNo ? "text-emerald-700" : "text-rose-700"}>CR {l.crCertNo ? "✓" : "✗"}</span>{l.crCertNo && <span className="text-[rgb(var(--muted))]"> · {l.crCertNo}</span>}</>}
                      </td>
