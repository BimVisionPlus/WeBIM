...
export function CreateForm({ projects }: { projects: ProjectOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-white" data-testid="open-create-form">+ Nhận lô VL</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="materialtrace-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <label className="text-xs"><span className="block text-slate-600">Mã lot</span><input required value={f.lotCode} onChange={(e) => setF({ ...f, lotCode: e.target.value })} placeholder="LOT-XXX-001" className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 font-mono" name="lotCode" /></label>
        <label className="text-xs"><span className="block text-slate-600">Loại</span><select value={f.category} onChange={(e) => setF({ ...f, category: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="category"><option value="XI_MANG">Xi măng</option><option value="THEP">Thép</option><option value="KINH">Kính</option><option value="GACH">Gạch</option><option value="BE_TONG_TUOI">BT thương phẩm</option><option value="SON">Sơn</option><option value="PHU_GIA">Phụ gia</option><option value="OTHER">Khác</option></select></label>
        <label className="text-xs md:col-span-2"><span className="block text-slate-600">Tên VL</span><input required value={f.materialName} onChange={(e) => setF({ ...f, materialName: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="materialName" /></label>
        <label className="text-xs"><span className="block text-slate-600">NSX</span><input required value={f.manufacturer} onChange={(e) => setF({ ...f, manufacturer: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="manufacturer" /></label>
        <label className="text-xs"><span className="block text-slate-600">Số lượng</span><input required value={f.quantity} onChange={(e) => setF({ ...f, quantity: e.target.value })} pattern="\d+(\.\d+)?" className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="quantity" /></label>
        ...
        <label className="text-xs"><span className="block text-slate-600">Số CR hợp quy</span><input value={f.crCertNo} onChange={(e) => setF({ ...f, crCertNo: e.target.value })} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5" name="crCertNo" /></label>
      </div>
      ...
      <div className="mt-3 flex justify-end"><button type="submit" disabled={busy} className="rounded bg-blue-600 px-4 py-1.5 text-sm font-medium text-white disabled:opacity-50" data-testid="submit-create">{busy ? "…" : "Nhận"}</button></div>
    </form>
  );
}
