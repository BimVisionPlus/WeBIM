  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">NLĐ đang làm</div><div className="mt-1 text-2xl font-bold text-emerald-700">{active}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Có mặt hôm nay</div><div className="mt-1 text-2xl font-bold">{todayInCount}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Chứng chỉ ATLĐ hết hạn</div><div className="mt-1 text-2xl font-bold text-rose-700">{hseExpired}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">CC hành nghề sắp hết ≤60d</div><div className="mt-1 text-2xl font-bold text-amber-700">{proExpiring}</div></CardBody></Card>
      </div>
      ...
      <Card className="mt-4">
        ...
        <CardBody className="p-0">
          {workers.length === 0 ? (
            <div className="p-6 text-center text-sm text-[rgb(var(--muted))]">Chưa có nhân lực. Bấm “Thêm NLĐ” để đăng ký công nhân.</div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[rgb(var(--line))]">
                {workers.map((w) => {
                  ...
                  return (
                    <tr key={w.id} className="hover:bg-[rgb(var(--raised))]" data-testid={`worker-${w.workerCode}`}>
                      <td className="p-2 font-mono text-xs">{w.workerCode}</td>
                      <td className="p-2 text-xs"><div className="font-medium">{w.fullName}</div>{w.idNo && <div className="text-[10px] text-[rgb(var(--muted))]">{w.idNo}</div>}</td>
                      <td className="p-2 text-xs">{w.trade}{w.level && <div className="text-[10px] text-[rgb(var(--muted))]">{w.level}</div>}</td>
                      <td className="p-2 text-xs">{w.org.name}</td>
                      <td className="p-2 text-xs">{w.hseCertNumber ? <><span className={hseOk ? "text-emerald-700" : "text-rose-700"}>{w.hseCertNumber}</span><div className="text-[10px] text-[rgb(var(--muted))]">{w.hseCertExpiry ? `hết ${formatDateVn(w.hseCertExpiry)}` : ""}</div></> : <span className="text-[rgb(var(--muted-2))]">Chưa có</span>}</td>
                      <td className="p-2 text-xs">{w.proLicenseNo ? <><span className={proOk ? "text-emerald-700" : "text-amber-700"}>{w.proLicenseNo}</span><div className="text-[10px] text-[rgb(var(--muted))]">{w.proLicenseExpiry ? formatDateVn(w.proLicenseExpiry) : ""}</div></> : <span className="text-[rgb(var(--muted-2))]">—</span>}</td>
      ...
      <Card className="mt-6">
        ...
        <CardBody className="p-0">
          {todayAttendance.length === 0 ? (
            <div className="p-6 text-center text-sm text-[rgb(var(--muted))]">Hôm nay chưa có check-in nào.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="border-b border-[rgb(var(--line))] bg-[rgb(var(--raised))] text-xs uppercase text-[rgb(var(--muted))]">
                <tr>
                ...
                {todayAttendance.map((a) => {
                  ...
                  return (
                    <tr key={a.id} className="hover:bg-[rgb(var(--raised))]">
                      ...
                      <td className="p-2 text-xs">{a.worker.fullName} <span className="text-[10px] text-[rgb(var(--muted))]">· {a.worker.trade}</span></td>
                      <td className="p-2 text-xs">{a.gateCode ?? "—"}</td>
                      <td className="p-2 text-xs">{methodLabel[a.method]}{a.faceMatchScore && <span className="text-[10px] text-[rgb(var(--muted))]"> · {Number(a.faceMatchScore).toFixed(3)}</span>}</td>
