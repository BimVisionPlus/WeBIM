  ...
  return (
    <AecModuleShell group="Tài chính kế toán" name="Bảng giao khoán cho đơn vị" subtitle="Khoán gọn từng hạng mục cho đội/nhà thầu phụ kèm tiến độ.">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Tổng giao khoán</div><div className="mt-1 text-2xl font-bold">{assignments.length}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Đang khoán</div><div className="mt-1 text-2xl font-bold text-blue-700">{active}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Giá trị đang khoán</div><div className="mt-1 text-2xl font-bold">{formatVnd(BigInt(totalActiveAmount))}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">TB hoàn thành</div><div className={`mt-1 text-2xl font-bold ${avgPct >= 80 ? "text-emerald-700" : "text-amber-700"}`}>{avgPct}%</div></CardBody></Card>
      </div>
      ...
      <Card className="mt-4">
        ...
        <CardBody className="p-0">
          {assignments.length === 0 ? (
            <div className="p-8 text-center text-sm text-[rgb(var(--muted))]">Chưa có bảng giao khoán nào. Bấm "Giao khoán" để bắt đầu.</div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[rgb(var(--line))]">
                {assignments.map((a) => {
                  ...
                  return (
                    <tr key={a.id} className="hover:bg-[rgb(var(--raised))]" data-testid={`row-assign-${a.id}`}>
                      <td className="p-2"><div className="font-medium">{a.contractorName}</div><div className="text-[10px] text-[rgb(var(--muted))]">{a.contractorOrg?.name ?? ""}</div></td>
                      ...
                      <td className="p-2 text-right"><div className="inline-flex items-center gap-2"><div className="h-1.5 w-16 rounded-full bg-[rgb(var(--line))] overflow-hidden"><div className={`h-full ${a.pctComplete >= 80 ? "bg-emerald-500" : "bg-blue-500"}`} style={{ width: `${a.pctComplete}%` }} /></div><span className="text-xs">{Math.round(a.pctComplete)}%</span></div></td>
      ...
      <div className="mt-2 text-[11px] text-[rgb(var(--muted))]">Mốc bàn giao: {assignments.length === 0 ? "—" : assignments.slice(0,3).map((a) => `${a.contractorName} (${formatDateVn(a.endDate)})`).join(" · ")}</div>
    </AecModuleShell>
  );
}
