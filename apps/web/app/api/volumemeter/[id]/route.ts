...
export async function DELETE(req: NextRequest, ctx: { params: { id: string } | Promise<{ id: string }> }) {
  const rl = await rateLimitGuard(req, { name: "volumemeter.delete" }); if (rl) return rl;
  try {
    const { id } = await ctx.params;
    const rec = await prisma.takeoffSheet.findUnique({ where: { id }, select: { id: true, projectId: true, state: true } });
    ...
    await prisma.takeoffSheet.delete({ where: { id } });
    await audit({ action: "volumemeter.delete", entityType: "TakeoffSheet", entityId: id, actorId: session.userId, projectId: rec.projectId, ...reqMeta(req) });
