    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/40 p-4" onClick={() => setOpen(false)}>
          <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="w-full max-w-2xl rounded-lg bg-white p-5 shadow-2xl" data-testid="edit-drawer">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              {fields.map((f) => {
                ...
                const common = "mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-xs";
                if (f.type === "select") {
                  return (
                    <label key={f.key} className={`text-xs ${cs}`}><span className="block text-slate-600">{f.label}</span>
                ...
                if (f.type === "textarea") {
                  return (
                    <label key={f.key} className={`text-xs ${cs || "md:col-span-3"}`}><span className="block text-slate-600">{f.label}</span>
                ...
                if (f.type === "money") {
                  return (
                    <label key={f.key} className={`text-xs ${cs}`}><span className="block text-slate-600">{f.label} (VND)</span>
            ...
            <div className="mt-3 flex justify-end gap-2">
              ...
              <button type="submit" disabled={busy} className="rounded bg-blue-600 px-3 py-1 text-xs font-medium text-white disabled:opacity-50" data-testid="edit-save">{busy ? "…" : "Lưu"}</button>
            </div>
          </form>
        </div>
      )}
