    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-40 grid place-items-center bg-slate-900/40 p-4" onClick={() => setOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-lg rounded-lg bg-white p-5 shadow-2xl" data-testid="digest-modal">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-semibold">Tóm tắt tuần — {label}</h3>
              <button onClick={() => setOpen(false)} className="text-sm text-slate-500">Đóng</button>
            </div>
            {busy && <div className="py-6 text-center text-sm text-slate-500">⏳ Groq Llama-3.3-70b đang xử lý…</div>}
            {!busy && summary && (
              <>
                <div className="whitespace-pre-line rounded border border-violet-200 bg-violet-50/40 p-3 text-sm text-slate-800" data-testid="digest-summary">{summary}</div>
                {model && <div className="mt-2 text-[10px] text-slate-500">Mô hình: {model}</div>}
              </>
            )}
            {!busy && fallback && (
              <div className="whitespace-pre-line rounded border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700" data-testid="digest-fallback">{fallback}</div>
            )}
          </div>
        </div>
      )}
