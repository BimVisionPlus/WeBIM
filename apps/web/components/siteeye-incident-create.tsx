  ...
  return (
    <>
      <button
        ...
        className="rounded-md bg-rose-600 px-3 py-2 text-sm font-medium text-[rgb(var(--inverse-ink))] hover:bg-rose-700"
      >
        🚨 Báo sự cố
      </button>
      {open && (
          ...
          <div className="w-full max-w-lg rounded-lg bg-[rgb(var(--surface))] p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="mt-4 space-y-3" onSubmit={onSubmit}>
              <label className="block">
                <span className="block text-xs font-medium text-[rgb(var(--ink-2))]">Thời điểm xảy ra *</span>
                <input
                  name="occurredAt"
                  type="datetime-local"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <label className="block">
                  <span className="block text-xs font-medium text-[rgb(var(--ink-2))]">Mức độ *</span>
                  <select
                    name="severity"
                    required
                    className="mt-1 w-full rounded-md border border-[rgb(var(--line-2))] px-2.5 py-1.5 text-sm"
              ...
              <label className="block">
                <span className="block text-xs font-medium text-[rgb(var(--ink-2))]">Vị trí</span>
                <input
                  name="location"
                  className="mt-1 w-full rounded-md border border-[rgb(var(--line-2))] px-2.5 py-1.5 text-sm"
              ...
              <label className="block">
                <span className="block text-xs font-medium text-[rgb(var(--ink-2))]">Số người bị thương</span>
                <input
                  name="injured"
                  type="number"
                  defaultValue={0}
              ...
              <label className="block">
                <span className="block text-xs font-medium text-[rgb(var(--ink-2))]">Mô tả sự cố *</span>
                <textarea
                  name="description"
                  required
                  rows={4}
              ...
              <label className="block">
                <span className="block text-xs font-medium text-[rgb(var(--ink-2))]">Biện pháp xử lý ngay</span>
                <textarea
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={busy}
                  className="rounded-md bg-rose-600 px-3 py-1.5 text-sm font-medium text-[rgb(var(--inverse-ink))] hover:bg-rose-700 disabled:opacity-50"
                >
