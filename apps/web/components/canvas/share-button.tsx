...
export function ShareButton({ sheetId }: { sheetId: string }) {
  ...
  return <div className="flex items-center gap-2"><button onClick={create} disabled={busy} className="rounded-lg bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-50">{busy ? "Đang tạo…" : "Chia sẻ để duyệt"}</button>{url && <input readOnly value={url} onFocus={(e) => e.currentTarget.select()} className="w-80 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs" />}{url && <span className="text-xs text-emerald-600">Đã sao chép</span>}{error && <span className="text-xs text-red-600">{error}</span>}</div>;
}
