    ...
    <span className="inline-flex items-center gap-2" data-testid={`project-row-actions-${id}`}>
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-[rgb(var(--inverse-bg))]/40 p-4" onClick={() => setOpen(false)}>
          <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-lg bg-[rgb(var(--surface))] p-5 shadow-2xl" data-testid="edit-project-form">
            ...
            <div className="space-y-3">
              ...
              <label className="block text-xs"><span className="text-[rgb(var(--muted))]">Tỉnh</span><input value={f.province} onChange={(e) => setF({ ...f, province: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" /></label>
              <label className="block text-xs"><span className="text-[rgb(var(--muted))]">Deadline</span><input type="date" value={f.endDate} onChange={(e) => setF({ ...f, endDate: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" /></label>
              <label className="block text-xs"><span className="text-[rgb(var(--muted))]">Trạng thái</span><select value={f.status} onChange={(e) => setF({ ...f, status: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5"><option value="PLANNING">Chuẩn bị</option><option value="IN_PROGRESS">Đang thi công</option><option value="HANDOVER">Bàn giao</option><option value="WARRANTY">Bảo hành</option><option value="CLOSED">Đã đóng</option></select></label>
            </div>
