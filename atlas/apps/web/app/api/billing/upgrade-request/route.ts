...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const subject = `[Atlas] Yêu cầu nâng cấp ${PLAN_LABEL[d.planCode] ?? d.planCode} — ${org.name}`;
    const html = `
    ...
    const BANK_HOLDER = process.env.BANK_HOLDER ?? "CTCP Atlas";

    // Reply to the customer too (best-effort)
    await sendEmail({
      to: d.contactEmail,
      subject: "Atlas — đã nhận yêu cầu nâng cấp",
