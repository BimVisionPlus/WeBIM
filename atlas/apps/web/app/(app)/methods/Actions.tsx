...
export function CreateForm({ projects, templates }: { projects: ProjectOpt[]; templates: TemplateOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-[rgb(var(--inverse-ink))]" data-testid="open-create-form">+ Tạo BPTC</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="methods-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Mã</span><input required value={f.code} onChange={(e) => setF({ ...f, code: e.target.value })} placeholder="BPTC-XXX-001" className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5 font-mono" name="code" /></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Loại</span><select value={f.category} onChange={(e) => setF({ ...f, category: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="category"><option value="COC">Cọc</option><option value="DAO_DAT">Đào đất/nền</option><option value="BE_TONG_KHOI">BT khối lớn</option><option value="KET_CAU">Kết cấu BTCT</option><option value="KET_CAU_THEP">Kết cấu thép</option><option value="MEP">MEP</option><option value="HOAN_THIEN">Hoàn thiện</option><option value="CAU_GIANG_GIO">Cẩu/giàn giáo</option><option value="HAN_CO_DIEN">Hàn nguy cơ cháy</option><option value="KHAC">Khác</option></select></label>
        ...
        <label className="text-xs md:col-span-2"><span className="block text-[rgb(var(--muted))]">Phạm vi</span><input required value={f.scope} onChange={(e) => setF({ ...f, scope: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="scope" /></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Template (optional)</span><select value={f.templateId} onChange={(e) => setF({ ...f, templateId: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="templateId"><option value="">Không dùng</option>{templates.map((t) => <option key={t.id} value={t.id}>{t.code}</option>)}</select></label>
      </div>
