  ...
  return (
    <AecModuleShell group="Hành chính" name="Theo dõi BHXH" subtitle="Sổ bảo hiểm xã hội cho người lao động theo Luật BHXH.">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Tổng bản ghi</div><div className="mt-1 text-2xl font-bold">{records.length}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Đang đóng</div><div className="mt-1 text-2xl font-bold text-emerald-700">{dangDong}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Chờ đăng ký</div><div className="mt-1 text-2xl font-bold text-amber-700">{choDangKy}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Mức đóng/tháng</div><div className="mt-1 text-2xl font-bold">{formatVnd(BigInt(totalMonthly))}</div></CardBody></Card>
      </div>
      ...
      <Card className="mt-4">
        ...
        <CardBody className="p-0">
          {records.length === 0 ? (
            <div className="p-8 text-center text-sm text-[rgb(var(--muted))]">Chưa có bản ghi BHXH. Bấm "Thêm bản ghi BHXH" để bắt đầu.</div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[rgb(var(--line))]">
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-[rgb(var(--raised))]" data-testid={`row-bhxh-${r.id}`}>
                      <td className="p-2"><div className="font-medium">{r.employeeName}</div><div className="text-[10px] text-[rgb(var(--muted))]">{r.org.name}</div></td>
