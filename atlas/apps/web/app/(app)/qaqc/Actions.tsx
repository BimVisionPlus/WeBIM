...
export function CreateForm({ projects, itps }: { projects: ProjectOpt[]; itps: ItpOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-[rgb(var(--inverse-ink))]" data-testid="open-create-form">+ Lên lịch check ITP</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="qaqc-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">ITP template</span><select value={f.templateId} onChange={(e) => setF({ ...f, templateId: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="templateId">{itps.map((t) => <option key={t.id} value={t.id}>{t.code}</option>)}</select></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Vị trí</span><input required value={f.location} onChange={(e) => setF({ ...f, location: e.target.value })} placeholder="Cọc P14 trục 3-A" className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="location" /></label>
      </div>
...
export function ResultActions({ id, result }: { id: string; result: string }) {
  ...
  return (
    <div className="flex flex-wrap gap-1" data-testid={`result-actions-${id}`}>
      ...
      <button onClick={() => go("REWORK")} disabled={busy === "REWORK"} className="rounded bg-amber-600 px-2 py-0.5 text-[10px] font-medium text-[rgb(var(--inverse-ink))] disabled:opacity-50" data-testid="action-REWORK">{busy === "REWORK" ? "…" : "Rework"}</button>
      <button onClick={() => go("WAIVED")} disabled={busy === "WAIVED"} className="rounded bg-[rgb(var(--line))] px-2 py-0.5 text-[10px] font-medium disabled:opacity-50" data-testid="action-WAIVED">{busy === "WAIVED" ? "…" : "Miễn"}</button>
    </div>
  );
}
