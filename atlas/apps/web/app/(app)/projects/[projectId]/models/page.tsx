    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {models.map((m) => (
            <Link key={m.id} href={`/projects/${params.projectId}/models/${m.id}`}>
              <Card className="h-full transition-shadow hover:shadow-md">
                ...
                <CardBody className="space-y-1 text-xs text-[rgb(var(--muted))]">
                  ...
                  {m.apsTranslationProgress < 100 && m.apsTranslationStatus !== "SUCCESS" && (
                    <div className="mt-2 h-1 w-full rounded bg-[rgb(var(--raised))]">
                      <div
          ...
          {models.length === 0 && (
            <Card>
              <CardBody className="py-10 text-center text-sm text-[rgb(var(--muted))]">Chưa có mô hình nào.</CardBody>
            </Card>
          )}
        </div>
      </section>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[rgb(var(--muted))]">Versioned: R0 / R1 / IFC. Supersedes chain giữ lịch sử thay đổi.</p>
          </div>
