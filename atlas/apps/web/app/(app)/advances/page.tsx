  ...
  return (
    <AecModuleShell group="Tài chính kế toán" name="Tạm ứng & thanh toán" subtitle="Sổ tạm ứng, thanh toán, hoàn ứng theo dự án và đơn vị nhận.">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Tổng giao dịch</div><div className="mt-1 text-2xl font-bold">{txns.length}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Tạm ứng chưa hoàn</div><div className="mt-1 text-2xl font-bold text-amber-700">{formatVnd(BigInt(totalTamUngOpen))}</div><div className="text-[10px] text-[rgb(var(--muted))]">{tamUngOpen.length} giao dịch</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Đã thanh toán</div><div className="mt-1 text-2xl font-bold text-blue-700">{formatVnd(BigInt(totalThanhToan))}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Đã hoàn ứng</div><div className="mt-1 text-2xl font-bold text-emerald-700">{formatVnd(BigInt(totalHoanUng))}</div></CardBody></Card>
      </div>
      ...
      <Card className="mt-4">
        ...
        <CardBody className="p-0">
          {txns.length === 0 ? (
            <div className="p-8 text-center text-sm text-[rgb(var(--muted))]">Chưa có giao dịch nào. Bấm "Tạm ứng / Thanh toán" để bắt đầu.</div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[rgb(var(--line))]">
                {txns.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className={`hover:bg-[rgb(var(--raised))] ${overdue ? "bg-amber-50/50" : ""}`} data-testid={`row-advance-${t.id}`}>
                      ...
                      <td className="p-2 font-mono text-xs">{t.txnNo ?? "—"}{t.project && <div className="text-[10px] text-[rgb(var(--muted))]">{t.project.key}</div>}</td>
                      <td className="p-2"><Badge variant={tm.variant}>{tm.vn}</Badge></td>
                      <td className="p-2"><div className="font-medium">{t.payeeName}</div><div className="text-[11px] text-[rgb(var(--muted))] line-clamp-1">{t.purpose}</div></td>
