...
export function CreateForm({ projects }: { projects: ProjectOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-[rgb(var(--inverse-ink))]" data-testid="open-create-form">+ Nhật ký TVGS hôm nay</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="supervise-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Ngày</span><input required type="date" value={f.logDate} onChange={(e) => setF({ ...f, logDate: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="logDate" /></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Ca</span><select value={f.shift} onChange={(e) => setF({ ...f, shift: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="shift"><option value="DAY">Ngày</option><option value="NIGHT">Đêm</option><option value="FULL">Cả ngày</option></select></label>
        <label className="text-xs md:col-span-3"><span className="block text-[rgb(var(--muted))]">Thời tiết</span><input value={f.weather} onChange={(e) => setF({ ...f, weather: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="weather" /></label>
        <label className="text-xs md:col-span-3"><span className="block text-[rgb(var(--muted))]">Công việc thi công</span><textarea required value={f.workItems} onChange={(e) => setF({ ...f, workItems: e.target.value })} rows={2} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="workItems" /></label>
        <label className="text-xs md:col-span-3"><span className="block text-[rgb(var(--muted))]">Quan sát chất lượng</span><textarea value={f.qualityNotes} onChange={(e) => setF({ ...f, qualityNotes: e.target.value })} rows={2} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="qualityNotes" /></label>
        <label className="text-xs md:col-span-3"><span className="block text-[rgb(var(--muted))]">Quan sát ATLĐ</span><textarea value={f.safetyNotes} onChange={(e) => setF({ ...f, safetyNotes: e.target.value })} rows={2} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="safetyNotes" /></label>
      </div>
    ...
    <div className="flex flex-wrap gap-1" data-testid={`actions-${id}`}>
      {actions.map((a) => (
        <button key={a.action} onClick={() => go(a.action)} disabled={busy === a.action}
          className="rounded bg-blue-600 px-2 py-0.5 text-[10px] font-medium text-[rgb(var(--inverse-ink))] disabled:opacity-50"
