...
export function CreateForm({ orgs }: { orgs: OrgOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-[rgb(var(--inverse-ink))]" data-testid="open-create-form">+ Tạo gói thầu</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="tenderforge-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Tổ chức</span><select required value={f.orgId} onChange={(e) => setF({ ...f, orgId: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="orgId">{orgs.map((o) => <option key={o.id} value={o.id}>{o.name}</option>)}</select></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Vai trò</span><select value={f.perspective} onChange={(e) => setF({ ...f, perspective: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="perspective"><option value="NHA_THAU">HSDT (Nhà thầu)</option><option value="BEN_MOI">HSMT (Bên mời)</option></select></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Mã</span><input required value={f.code} onChange={(e) => setF({ ...f, code: e.target.value })} placeholder="HSDT-2026-XXX" className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5 font-mono" name="code" /></label>
        ...
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Loại gói</span><input required value={f.packageType} onChange={(e) => setF({ ...f, packageType: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="packageType" /></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Hình thức</span><input required value={f.selectionMethod} onChange={(e) => setF({ ...f, selectionMethod: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="selectionMethod" /></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Giá trị (VND)</span><input value={f.estimatedValueVnd} onChange={(e) => setF({ ...f, estimatedValueVnd: e.target.value })} pattern="\d+" className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="estimatedValueVnd" /></label>
      </div>
