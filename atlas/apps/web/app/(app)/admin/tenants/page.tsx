  ...
  if (!session.isSuperAdmin) {
    return (
      <div className="p-12 text-center">
        ...
        <p className="mt-2 text-sm text-[rgb(var(--muted))]">Liên hệ Atlas team để được cấp quyền truy cập CRM.</p>
      </div>
    );
  }
  ...
  return (
    <AecModuleShell group="Admin" name="Tenant CRM — Pipeline khách dùng thử" subtitle="Quản lý prospect sandbox: theo dõi, gia hạn, chuyển trả phí. Chỉ super-admin truy cập.">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Active</div><div className="mt-1 text-2xl font-bold text-emerald-700">{totalActive}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Sắp hết hạn (≤3 ngày)</div><div className="mt-1 text-2xl font-bold text-amber-700">{expiringSoon}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Đã hết hạn</div><div className="mt-1 text-2xl font-bold text-[rgb(var(--ink-2))]">{totalExpired}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Chuyển trả phí</div><div className="mt-1 text-2xl font-bold text-blue-700">{totalConverted}</div></CardBody></Card>
        <Card><CardBody className="py-3"><div className="text-xs text-[rgb(var(--muted))]">Provision 30d</div><div className="mt-1 text-2xl font-bold">{recentProvisions}</div><div className="text-[10px] text-[rgb(var(--muted))]">{last30Visits} lượt truy cập</div></CardBody></Card>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[rgb(var(--line))]">
                {tenants.map((t: any) => {
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-[rgb(var(--raised))]" data-testid={`row-tenant-${t.slug}`}>
                      <td className="p-2">
                        <div className="font-medium">{t.prospectName ?? "(no name)"}</div>
                        <div className="text-[10px] text-[rgb(var(--muted))]">{t.prospectEmail ?? "—"}</div>
                        {t.prospectCompany && <div className="text-[10px] text-[rgb(var(--muted))]">{t.prospectCompany} · {t.prospectIndustry ?? "—"}</div>}
                      </td>
                      <td className="p-2">
                      ...
                      <td className="p-2 text-right">
                        ...
                        {t.lastVisitedAt && <div className="text-[10px] text-[rgb(var(--muted))]">{formatDateVn(t.lastVisitedAt)}</div>}
                      </td>
                      ...
                      <td className="p-2 text-xs">
                        ...
                        {daysLeft !== null && daysLeft >= 0 && (
                          <div className={`text-[10px] ${daysLeft <= 3 ? "text-rose-700" : "text-[rgb(var(--muted))]"}`}>
                            còn {daysLeft} ngày
                          </div>
                        )}
                      </td>
      ...
      <div className="mt-4 rounded-lg border border-[rgb(var(--line))] bg-[rgb(var(--raised))] p-3 text-xs text-[rgb(var(--muted))]">
