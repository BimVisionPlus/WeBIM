...
export function CreateForm({ orgs }: { orgs: OrgOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-[rgb(var(--inverse-ink))]" data-testid="open-create-form">+ Đăng ký năng lực</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="registry-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Tổ chức</span><select required value={f.orgId} onChange={(e) => { const o = orgs.find((x) => x.id === e.target.value); setF({ ...f, orgId: e.target.value, legalName: o?.name ?? "", mst: o?.mst ?? "" }); }} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="orgId">{orgs.map((o) => <option key={o.id} value={o.id}>{o.name}</option>)}</select></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Tên pháp lý</span><input required value={f.legalName} onChange={(e) => setF({ ...f, legalName: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="legalName" /></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">MST</span><input value={f.mst} onChange={(e) => setF({ ...f, mst: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="mst" /></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Hạng</span><select value={f.capabilityClass} onChange={(e) => setF({ ...f, capabilityClass: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="capabilityClass"><option value="HANG_I">Hạng I</option><option value="HANG_II">Hạng II</option><option value="HANG_III">Hạng III</option><option value="CHUA_PHAN_HANG">Chưa phân hạng</option></select></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Số CCNL</span><input value={f.capabilityNo} onChange={(e) => setF({ ...f, capabilityNo: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="capabilityNo" /></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">KS chính</span><input type="number" min={0} value={f.charteredEng} onChange={(e) => setF({ ...f, charteredEng: Number(e.target.value) })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="charteredEng" /></label>
      </div>
...
export function RowActions({ id, blacklisted }: { id: string; blacklisted: boolean }) {
  ...
  return (
    <div className="flex flex-wrap gap-1" data-testid={`actions-${id}`}>
      ...
      <button onClick={() => go("UPDATE_RATING")} disabled={busy === "UPDATE_RATING"} className="rounded bg-[rgb(var(--raised))] px-2 py-0.5 text-[10px] font-medium disabled:opacity-50" data-testid="action-UPDATE_RATING">{busy === "UPDATE_RATING" ? "…" : "Sửa rating"}</button>
    </div>
  );
}
