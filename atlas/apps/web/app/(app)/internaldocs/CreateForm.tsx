...
export function CreateForm({ orgs }: { orgs: OrgOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-[rgb(var(--inverse-ink))]" data-testid="open-create-form">+ Thêm văn bản nội bộ</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="internaldoc-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Số văn bản</span><input required value={f.docNo} onChange={(e) => setF({ ...f, docNo: e.target.value })} placeholder="QĐ-12/2026" className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="docNo" /></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Loại</span><select value={f.category} onChange={(e) => setF({ ...f, category: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="category">{categories.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}</select></label>
        ...
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Ngày ban hành</span><input type="date" value={f.issuedAt} onChange={(e) => setF({ ...f, issuedAt: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="issuedAt" /></label>
        <label className="text-xs md:col-span-3"><span className="block text-[rgb(var(--muted))]">Nội dung</span><textarea value={f.body} onChange={(e) => setF({ ...f, body: e.target.value })} rows={3} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="body" /></label>
      </div>
