...
export function ClashScan({ projects }: { projects: ProjectOpt[] }) {
  ...
  return (
    <div className="rounded-lg border border-[rgb(var(--line))] bg-[rgb(var(--raised))] p-3" data-testid="clash-scan">
      <div className="flex flex-wrap items-center gap-3">
        <span className="text-xs font-medium text-[rgb(var(--muted))]">Kiểm tra xung đột giữa các bộ môn (kết cấu · MEP · kiến trúc)</span>
        <select value={projectId} onChange={(e) => setProjectId(e.target.value)} className="rounded border border-[rgb(var(--line-2))] px-2 py-1 text-xs" data-testid="clash-scan-project">
          {projects.map((p) => <option key={p.id} value={p.id}>{p.key}</option>)}
        </select>
        <button onClick={run} disabled={busy} className="rounded bg-blue-600 px-3 py-1.5 text-xs font-medium text-[rgb(var(--inverse-ink))] disabled:opacity-50" data-testid="clash-scan-run">{busy ? "Đang dò…" : "Kiểm tra xung đột"}</button>
