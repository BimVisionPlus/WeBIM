...
export function CreateForm({ projects }: { projects: ProjectOpt[] }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-blue-600 px-3 py-1.5 text-sm font-medium text-[rgb(var(--inverse-ink))]" data-testid="open-create-form">+ Tạo hồ sơ PCCC</button>;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-blue-200 bg-blue-50/50 p-4" data-testid="pccc-create-form">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Giai đoạn</span><select value={f.stage} onChange={(e) => setF({ ...f, stage: e.target.value })} className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="stage">{stages.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}</select></label>
        <label className="text-xs"><span className="block text-[rgb(var(--muted))]">Mã hồ sơ (PC07/C06)</span><input value={f.applicationCode} onChange={(e) => setF({ ...f, applicationCode: e.target.value })} placeholder="để trống nếu chưa có" className="mt-1 w-full rounded border border-[rgb(var(--line-2))] px-2 py-1.5" name="applicationCode" /></label>
